...
export default async function globalSetup(config: FullConfig): Promise<void> {
  ...
  const api = process.env.AXIOM_BASE ?? 'http://127.0.0.1:5175';
  ...
  process.env.AXIOM_TEST_TOKEN = token;
  ...
  await ctx.addInitScript((t) => localStorage.setItem('axiom_token', t), token);
  ...
  process.env.AXIOM_E2E_TOKEN = e2eToken;
  const ctx2 = await browser.newContext({ baseURL });
  await ctx2.addInitScript((t) => localStorage.setItem('axiom_token', t), e2eToken);
  const p2 = await ctx2.newPage();
  await p2.goto('/');
  await p2.getByTestId('new-chat').waitFor({ timeout: 15_000 });
  await ctx2.addCookies([
    { name: 'axiom_token', value: e2eToken, url: baseURL },
    { name: 'axiom_token', value: e2eToken, url: api },
  ]);
