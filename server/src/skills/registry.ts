...
export const SKILL_REGISTRY: SkillMeta[] = [
  {
    ...
    metaTokens: 98,
    fullTokens: 4200,
    helper: 'build_pptx.py · extract_office.py',
    validators: ['openxml-audit schema', 'python-pptx round-trip', 'placeholder grep', 'thumbnail grid (when soffice present)'],
    tier: '12B → Bedrock',
    note: 'Writes: model emits slide JSON under constrained decoding; helper fills branded .potx placeholders. Reads: an uploaded deck extracts to numbered slides with bullets, tables, chart series and speaker notes — read_document opens it on demand.',
  },
  {
    id: 'docx',
    name: 'Documents',
    ...
    icon: 'file-text',
    colorToken: 'blue',
    triggers: 'document · report · letter · redline · read',
    metaTokens: 92,
    fullTokens: 3800,
  ...
  },
  {
    ...
    colorToken: 'green',
    triggers: 'spreadsheet · model · budget · forecast · read',
    metaTokens: 104,
    fullTokens: 4600,
  ...
  },
  {
    ...
    icon: 'box',
    colorToken: 'purple',
    triggers: 'define a product · product concept · capability',
    metaTokens: 96,
    fullTokens: 2600,
    helper: 'projection engine',
    validators: [
      'Schema (constrained + ajv)',
      'Completeness — next gate',
      'Spine — Knowledge Core',
      'Collision — Knowledge Core',
      'Dependencies — Knowledge Core',
    ],
    tier: '12B → Bedrock',
    note: 'One object, many projections: the master JSON is the source of truth; concept page, BRD, gate deck, diagram, prototype, and build bundle are deterministic renders. KC checks degrade to amber offline.',
  },
  {
    id: 'react',
