...
| FR-1.3 | Self-healing Sonnet slot | `probeSonnet()` (boot/connect/refresh) issues a 1-token invoke of Sonnet 5. Refused → the slot binds to Sonnet 4.6 (`us.anthropic.claude-sonnet-4-5-20250929-v1:0`) with the honest label "Sonnet 5 pending AWS activation"; cleared → auto-upgrades. Setting key `sonnetResolved`. | ✅ (Sonnet 5 agreement ACTIVE, runtime still AWS-gated) |
...
| FR-5.3c | Project workspace | Opening a project (card → Open) enters a claude.ai-style workspace: project instructions (inline-editable), Knowledge + Project-memory buttons, the project's chat list, and "New chat in this project". Chats created here pass their projectId EXPLICITLY (no reliance on the async active-project setting) so their memory/knowledge scope is unambiguous. | ✅ |
| FR-5.3d | Scope indicator | The chat header shows a project chip ("⌂ {Project}") whose tooltip states the chat's memory & knowledge are scoped to that project. | ✅ |
