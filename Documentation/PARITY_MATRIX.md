...
streamable-HTTP-by-URL variant, X3b/KaTeX) — **P2's local gap fixed 2026-07-21,
see FX-24**; X3b/KaTeX remains the sole standing `@red`. Several rows below that were
...
**2026-07-19:** first-ever end-to-end exercise of the product (PRD) state
machine and its projections (new §9 below) — a real product-owner workflow
that had create/edit/version coverage but had never once walked the gated
transitions, the office projections, the model-generated prototype, or the
bundle export. Found and fixed THREE real product bugs on first contact
(docx/pptx projection payload shapes never matched the real schemas;
prototype_react's token budget was 8× too small for the react apps this same
pipeline is measured to produce) plus one router regression (C9 addendum —
"markdown one-pager PRD" fell through the deliverable-noun regex and
misrouted into editing an unrelated prior artifact). See FIXLOG FX-21/FX-22.
Also added `project-owner-prototypes.spec.ts` (M2 addendum): a full
product-owner session — JSX prototype, PRD one-pager, stakeholder docx — in
one project, which is what surfaced the router regression.
...
| C9 | md artifact | 🟢 | parity/c5-c12-artifacts.spec.ts ✓✓✓ 2026-07-18 | Regressed then re-fixed 2026-07-18: "Create a markdown document: a project readme..." misrouted to create-docx — the router's format-decisive-word check (added for a prior fix) had bare "markdown"/"md" as always-decisive, but people also say "a markdown table" to describe a chat reply's own inline formatting, not a request to create a file. Narrowed to a regex requiring markdown to name the deliverable itself. 3/3 consecutive, full 305-case routing dataset re-run clean. **2026-07-19 addendum:** that regex's noun list was still too narrow — "Create a markdown one-pager PRD for the loan payment calculator" matched none of its 5 nouns, fell through to noun-scoring, where "calculator" (a create-react-app noun describing the SUBJECT, not the format) outscored everything, and a prior react artifact in the same conversation then triggered Step 8's edit-inference, misrouting a new markdown request into an EDIT of that unrelated artifact. Broadened the noun list (one-pager/prd/spec/outline/notes/guide/plan/brief/writeup/report/memo). 305-case routing dataset re-verified clean (unambiguous + edit-vs-describe 100% all 3 tiers); project-owner-prototypes.spec.ts 3/3 |
...
| P2 | remote streamable-HTTP MCP add → tools → invoke, DEPLOYED | 🟢 | local spec ✓✓✓ 2026-07-21 (de-quarantined, `@red` removed) + DEPLOYED live run 2026-07-14 (deepwiki-sse logs archived) | REAL public server (mcp.deepwiki.com) added by URL on the deployed Lambda → connected → read_wiki_structure invoked in chat → grounded answer. The queued fix landed (FX-24): the "hardcoded p1" note above undersold it — the real bug was scoping new installs to the stale `activeProjectId` setting instead of the neutral General project every other unscoped action uses, compounded by a second bug where re-adding a connector under the same name left ambiguous duplicate rows that `installFor()`'s `.find()` could resolve to the wrong (stale-scoped) one. Both fixed; no manual toggle needed after adding |
...
| W3 | URL fetch → grounded answer with citation | 🟢 | parity/w2-w4.spec.ts 2026-07-14 | pasted URL fetched, heading answered verbatim (citation rendering counted under W2). **2026-07-19 note:** a full-suite regression scan (unrelated session — verifying `product-lifecycle.spec.ts`/router changes touched none of this workflow's code) surfaced an intermittent failure: Claude Haiku 4.5 sometimes declines to fetch `https://example.com`, reasoning it's "a placeholder domain" rather than a real page, and lectures instead of calling the tool (~25-30% of isolated re-runs, confirmed via 11 runs). Routing to `fetch-url-then-answer` fires correctly every time — this is model judgment, not a code defect, same class as R5/X3's documented instruction-compliance variance. Not fixed (a live model can't be made 100% deterministic); noted here rather than silently passed over |
...
## 9 · Product (PRD) lifecycle

Amendment 1 (`server/src/pipeline/product.ts` / `projections.ts`): a product owner's PRD as a structured artifact with a gated state machine and deterministic projections — capability the codebase had built but, before 2026-07-19, had **never once been exercised past create/edit/version.** Three of five rows below were found completely broken on first real use (evidence, not speculation: every projection kind that shells a real office builder failed on its first attempt).

| id | feature | status | evidence | notes |
|---|---|---|---|---|
| PRD1 | state machine: proposed→endorsed→specified→built→operating, each transition gated on real payload content, forward-one-step only, operating requires a manual note | 🟢 | live-smoke/product-lifecycle.spec.ts ✓✓✓ 2026-07-19 (16/16 × 3) | First-ever exercise of this gating end-to-end. No product bug in the state machine itself — every gate (`transitionRules`) fired correctly once given a real precondition; the one false alarm was this test's own creation prompt (the model proactively filled `benefit_hypothesis` unasked, since nothing told it not to — fixed by making the setup prompt explicit about what to withhold, not a product issue) |
| PRD2 | local projections: concept_md, concept_docx, brd_docx, gate_pptx, context_mermaid — real files opened with the real parser (python-docx/pptx) | 🟢 | live-smoke/product-lifecycle.spec.ts ✓✓✓ 2026-07-19 | **FOUND BROKEN, FIXED:** `toConceptDocx`/`toBrdDocx` emitted a `{sections: [...]}` payload shape; `skills/docx/schema.json` (the schema `build_docx.py` actually validates against) requires flat `{blocks: [...]}` — every concept_docx/brd_docx generation failed its very first real call. Separately, `toGatePptx` emitted `{layout, bullets, col_left/col_right}`; `skills/pptx/schema.json` requires `{archetype, title, speaker_notes, columns}` PLUS a word-count doctrine no JSON schema can express (`scripts/office/validate_common.py`: any single bullet/column item >12 words fails, and a "content" slide's title+bullets/columns must total ≤40 words) — the original slice(0,8)-bullet, full-paragraph-as-bullet approach violated both on first use. Rewrote both transforms against the real schemas, with a dynamic word-budget fitter (not a guessed fixed item count) for pptx |
| PRD3 | prototype_react: the one model-assisted projection — a real clickable prototype generated FROM the PRD | 🟢 | live-smoke/product-lifecycle.spec.ts ✓✓✓ 2026-07-19 | **FOUND BROKEN, FIXED:** hardcoded `maxTokens: 3072` — a real multi-file react app (measured ~18k output tokens elsewhere in this same pipeline) truncated mid-token on every attempt ("the model hit its output ceiling"). Switched to `officeMaxTokens()`, the same 24k-capped, model-clamped budget every other real-document call site in this pipeline already uses |
| PRD4 | bundle export: real zip (definition.json, acceptance/criteria.md+json, context/decisions.md, CLAUDE.md) — required before the built gate, downloadable via `/artifacts/:id/bundle` | 🟢 | live-smoke/product-lifecycle.spec.ts ✓✓✓ 2026-07-19 | zip contents verified via `zipfile`, not just a 200 status |
| PRD5 | push projections (confluence_page / jira_epics) degrade to local-only, honestly, when no connector is installed | 🟢 | live-smoke/product-lifecycle.spec.ts ✓✓✓ 2026-07-19 | request itself succeeds (never a 500); `status: 'local'` + a "connect X to push" note, matching the existing P2/pushConnector local-degrade pattern elsewhere in the matrix |
